...
where rownum <=10
    ...
    /*
Top 10 - Average Pledged Amounts, Goal Amounts, and Backers by Sub Categories and Main Categories
*/
SELECT *
FROM(SELECT CAT.MAINCATEGORY, CAT.SUBCATEGORY, ROUND(AVG(PRO.PLEDGEDAMOUNT), 1), 
ROUND(AVG(PRO.GOALAMOUNT), 1), ROUND(AVG(PRO.BACKERS), 1)
FROM PROJECT_T PRO, COUNTRY_T COU, CATEGORY_T CAT, CALENDAR_T CAL
WHERE PRO.CATEGORYID = CAT.CATEGORYID
    AND PRO.COUNTRYCODE = COU.COUNTRYCODE
    AND PRO.DEADLINE = CAL.DATES
    AND PRO.PROJECTSTATE = 'successful'
GROUP BY CAT.MAINCATEGORY, CAT.SUBCATEGORY
ORDER BY ROUND(AVG(PRO.PLEDGEDAMOUNT), 1) DESC)
WHERE ROWNUM <= 10;
